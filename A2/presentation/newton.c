...
{   
    ...
    sprintf(c_string1, "newton_attractors_x%d.ppm", deg);
    sprintf(c_string2, "newton_convergence_x%d.ppm", deg);
...
void newton(int deg, int *lroots, int *liter, int r_ix, int res, double *rep, double *imp){
    ...
    double step_size_col = 1 / n_threads;     // Step size column direction

    double row_im = 2 - 4*r_ix/(double)res;
    double ires = 1/(double)res;

    int ix_r;       // placeholders for root-index and iterations
    int iter_p;
    
    long long int roof = 10000000000;

    double complex z;
    double complex i_z;     // invers of z
    ...
    for(size_t cp = 0; cp < res; ++cp){
        ...
        ix_r = 0;
        iter = 0;
        z = -2 + 4*((double) cp*ires) + row_im*I;

        while(stepCon)
        {   
            if(iter > 500){    // The points around 0 + i0 have a tendency to continue for a long time, this stops that
                iter_p = iter;
                stepCon = false;
                break;
            }
            origin = creal(z)*creal(z) + cimag(z)*cimag(z);         // Calc. distance to origin
            if(origin < 0.000001){
                iter_p = iter;
                stepCon = false;
                break;
            ...
            if(origin - 1 < 0.001){
                for(size_t ir = 0; ir < deg; ++ir){
                    ...
                    if((dist < 0.000001) || (creal(z) > roof) || (cimag(z) > roof)){   // Checks if conditions are met
                        ix_r = ir;
                        iter_p = iter;
                        stepCon = false;
                        break;
            ...
            if(stepCon){        // So we dont take an unneccessary step if conditions are met
                switch(deg)
                {
                case 1:
                    z = 1;
        ...
        lroots[cp] = ix_r;      // Saves the root-index and number of iterations for point cp
        liter[cp] = iter_p;
    }
}
